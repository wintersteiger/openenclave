    ...
    _In_opt_ const struct timeval* a_Timeout)
{
    ...
    if (oe_result != OE_OK)
    {
        OE_TRACE_ERROR("result = %s", oe_result_str(oe_result));
        return 0;
    }

    if (result.error != 0)
    {
        OE_TRACE_ERROR("result.error = %d", result.error);
        return 0;
    }
    ...
    _In_ size_t a_uiBufferLength)
{
    ...
    {
        OE_TRACE_ERROR("OE_SOCKET_ERROR");
    ...
    if (oe_result != OE_OK)
    {
        OE_TRACE_ERROR("oe_result = %s", oe_result_str(oe_result));
    ...
    _Inout_ int* optlen)
{
    ...
    if ((unsigned long)*optlen > sizeof(result.buffer))
    {
    ...
    _In_ int protocol)
{
    if (network_security != OE_NETWORK_UNTRUSTED)
    {
        OE_TRACE_ERROR("network_security != OE_NETWORK_UNTRUSTED");
    ...
    _In_ int flags)
{
    ...
    {
        OE_TRACE_ERROR("ocall_recv failed");
        sock_error = OE_ENETDOWN;
    }
    oe_wsa_set_last_error(OE_NETWORK_UNTRUSTED, (int)sock_error);

    if (sock_error != OE_SOCKET_OK)
        OE_TRACE_ERROR("sock_error =%d", sock_error);
    else
        OE_TRACE_INFO("bytesReceived =%d", bytesReceived);

    return (sock_error != OE_SOCKET_OK) ? OE_SOCKET_ERROR : bytesReceived;
}

ssize_t oe_recvfrom( oe_socket_t s, void* buf, size_t len, int flags,
                    struct sockaddr *src_addr, oe_socklen_t addrlen)
{
    ...
    oe_result = ocall_recvfrom(&bytesReceived, s, buf, len, flags, src_addr, addrlen, &sock_error);
    if ((oe_result != OE_OK) ||
        (bytesReceived > 0 && (size_t)bytesReceived > len))
    {
        sock_error = OE_ENETDOWN;
    ...
    _In_ int flags)
{
    ...
    if (oeResult != OE_OK)
    {
        OE_TRACE_ERROR("oeResult = %s", oe_result_str(oeResult));
        result.error = OE_ENETDOWN;
        result.bytesSent = 0;
    }
    oe_wsa_set_last_error(OE_NETWORK_UNTRUSTED, (int)result.error);
    ...
    _Inout_ int* a_pAddrLen)
{
    ...
    {
        OE_TRACE_ERROR("ocall_accept failed", oe_result_str(oe_result));
        result.error = OE_ENETDOWN;
        addrlen = 0;
    }
    else
    {
        memcpy(a_SockAddr, result.addr, (unsigned long)result.addrlen);
...
void oe_freeaddrinfo(_In_ addrinfo* ailist)
{
    addrinfo* ai;
    addrinfo* next;
...
int oe_getaddrinfo(
    ...
    _In_ const struct addrinfo* pHints,
    _Out_ addrinfo** ppResult)
{
    OE_TRACE_VERBOSE("oe_getaddrinfo: pNodeName = [%s] pServiceName=[%s]", pNodeName, pServiceName);
    if (network_security != OE_NETWORK_UNTRUSTED)
    {
        return OE_UNSUPPORTED;
    }
    ...
    {
        int bytesReceived = (int)result.addressCount * (int)(sizeof(addrinfo_Buffer));
        buf = malloc((size_t)bytesReceived);
        if (buf == NULL)
        {
        ...
        {
            uStatus =
                pull_host_buffer_data(result.hMessage, buf, (size_t)bytesReceived);
            if (uStatus != OE_OK)
            {
                result.error = OE_ENOTRECOVERABLE;
            }
        ...
        {
            if ((unsigned long)(response[i].ai_addrlen) > sizeof(oe_sockaddr_storage) ||
                (unsigned long)(response[i].ai_addrlen) > sizeof(response[i].ai_addr))
            {
                result.error = OE_ENOMEM;
                break;
            }
            ...
            OE_TRACE_VERBOSE("ai->ai_family=[0x%x]", (int)(ai->ai_family));
            OE_TRACE_VERBOSE("ai->ai_flags=[0x%x]", (int)(ai->ai_flags));
            OE_TRACE_VERBOSE("ai->ai_socktype=[0x%x]", (int)(ai->ai_socktype));
            OE_TRACE_VERBOSE("ai->ai_addrlen=[0x%x]", (int)(ai->ai_addrlen));
            OE_TRACE_VERBOSE("ai->ai_addr=[0x%p]", ai->ai_addr);
