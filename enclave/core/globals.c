 ...
 *
 *        The compiler always generated RIP relative addressing for static
 *        global variables since it knows that these variables cannot be
 *        addressed by other compilation units:
 *
 *           get_g1:
 *              movl g1(%rip), %eax
 *              ret
 ...
 *
 *****************************************************************************
 * Calculating enclave base address securely
 *
 ...
 *
 * Strategy 2:
 *    Define a variable called oe_enclave_rva (relative virtual address).
 *    The oe_enclave_rva is supposed to be set to the offset of that variable
 *    in the enclave image (i.e. as if the enclave was loaded to memory
 *    address 0). This offset value is a constant and therefore the enclave
 *    is signed after setting oe_enclave_rva to this value.
 *
 *    Actual runtime address of oe_enclave_rva minus the predetermined offset
 *    of oe_enclave_rva gives the runtime base address of the enclave:
 *       enclave-base-address = &oe_enclave_rva - oe_enclave_rva;
 *
 *    If the host sets oe_enclave_rva to an arbitrary value, then the loading
 *    the enclave will fail since the signature (generated with incorrect
 *    oe_enclave_rva value) will not match the signature embedded by the
 *    oesign tool (using the correct oe_enclave_rva value).
 *
  *****************************************************************************
 ...
 * Relocations require enclave base address to be added to each relocation
 * entry (see reloc.c in linux/windows).
 * The Global Offset Table (GOT) is updated via performing the relocations.
 * This means that the oe_enclave_rva variable and other variables used for
 * relocations must not themselves be placed in the GOT. The simplest
 * (though not the only) way to enfore RIP relative addressing for
 * put all the relocation related variables into static global struct.
 **/

static uint64_t oe_enclave_rva;
static uint64_t oe_reloc_rva;
static uint64_t oe_reloc_size;
...
{
/*
 * Note: The reference to &oe_image_info will be compiled
 ...
 */
#if defined(__linux__)
    return (uint8_t*)&oe_enclave_rva - oe_enclave_rva;
#else
