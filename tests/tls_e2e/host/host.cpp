...
void *client_thread(void *arg)
{
    ...
    // In the no-fault-injection test case, the client thread should return cleanly (0)
    if (!client_config->args.fail_cert_verify_callback &&
        !client_config->args.fail_enclave_identity_verifier_callback &&
        !client_config->args.fail_oe_verify_tls_cert &&
...
int run_scenarios_tests()
{
    ...
    test_cases_config_t unittests_configs[4] = {
                                        {"positive_test", {false, false, false}},
                                        {"negative_fail_oe_verify_tls_cert", {true, false, false}},
                                        {"negative_fail_cert_verify_callback", {false, true, false}},
                                        {"negative_fail_enclave_identity_verifier_callback", {false, false, true}}
                                    };
    test_configs.server.enclave = g_server_enclave;
    test_configs.client.enclave = g_client_enclave;
    for (test_target_t i=server_target; i<max_test_target_count; i = test_target_t(i+1))
    {
        for (test_config_type_t j=config_type_no_fault_injection;
              j<max_test_config_type_count; j = test_config_type_t(j+1))
        {
            ...
            if (i == server_target)
            {
                ...
                test_configs.client.args = unittests_configs[config_type_no_fault_injection].args;
            }
            else
            {
                test_configs.server.args = unittests_configs[config_type_no_fault_injection].args;
                test_configs.client.args = unittests_configs[j].args;
            }
            printf("Test case: %s:%s\n", (i == server_target) ? "Server": "Client", unittests_configs[j].name);
